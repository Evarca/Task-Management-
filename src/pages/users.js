...
function usersPage(){
  ...
  ${filterBar(
     `<div style="position:relative;flex:1;min-width:170px"><span style="position:absolute;left:10px;top:50%;transform:translateY(-50%);color:var(--c-text-3);pointer-events:none">${ic('search','w-4 h-4')}</span><input id="usr-q" oninput="S.search=this.value;App._searchRR('usr-q')" value="${esc(S.search)}" placeholder="Search people by name or email…" class="ui-input" style="padding-left:34px;min-height:0;height:34px;font-size:13px"/></div>`
    +filterSelect('dep','All departments',DB.departments.map(d=>d.name),S.filters.dep)
    +filterSelect('stat','Any status',['Active','Inactive'],S.filters.stat)
    +((S.search||S.filters.dep||S.filters.stat)?`<button onclick="S.search='';['dep','stat'].forEach(k=>delete S.filters[k]);rr()" class="ui-btn ui-btn-ghost ui-btn-sm">Clear</button>`:'')
    +filterCount(list.length+' of '+scopedUsers('employees').length))}
...
App.saveUser=async(id)=>{
  ...
  if(id){
    ...
    _acPushHrm(u); // directory details live on u.hrm — push NOW, not on the debounce
  } else {
...
App.togUser=(id)=>{
  ...
  // Deactivating someone must not strand work that is still pointing at them: their pending
  // approvals stay visible to the approver, but unfinished tickets are unassigned so they
  // don't sit in a queue nobody is watching.
  if(!enabling){
    let freed=0;
    (DB.tickets||[]).filter(t=>t.assignedTo===id&&(t.status==='Open'||t.status==='In Progress')).forEach(t=>{
      t.assignedTo=null;freed++;
      sb.from('tickets').update({assigned_to:null}).eq('id',t.id).then(()=>{}).catch(()=>{});
    });
    if(freed){hlog('Tickets unassigned on deactivation',fullName(u)+' ('+freed+')');toast(freed+' open ticket'+(freed===1?'':'s')+' unassigned','warn');}
  }
  saveDB();render();
...
App.delUser=async(id)=>{
  ...
  // checklists, pending approvals and open tickets,
